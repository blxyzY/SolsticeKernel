...
/* === KERNEL 4.19 COMPATIBILITY SHIM ===
 *
 * arch_scale_cpu_capacity() lost its "struct sched_domain *sd" first
 * argument in mainline v5.1 (commit 7673c8a4c374, "sched/topology: Add
 * partition_sched_domains_locked()" era cleanup). Vanilla/mainline 4.19
 * still uses the OLD two-argument prototype:
 *
 *     unsigned long arch_scale_cpu_capacity(struct sched_domain *sd, int cpu)
 *
 * Some vendor Android common kernels (msm-4.19, exynos 4.19 branches with
 * EAS backported, etc.) already carry the newer single-argument form.
 * Check the prototype in your tree's include/linux/arch_topology.h or
 * kernel/sched/sched.h and flip RFX_OLD_ARCH_CAP_API below to match:
 *   1 -> two-argument form (vanilla 4.19, most msm-4.19/exynos trees)
 *   0 -> single-argument form (tree already backported the 5.1+ API)
 */
...
/* CPUFREQ_NEED_UPDATE_LIMITS was introduced alongside the newer
 * cpufreq_driver flags API and doesn't exist in 4.19's cpufreq.h.
 * rfx_driver_test_flags() below is a permanent stub (always returns
 * false), so this value is never actually evaluated against real driver
 * flags — it only needs to compile. */
...
{
	unsigned long cap = rfx_arch_cap(cpu);
			     ...
			     unsigned long util, unsigned long max_cap, u64 time)
{
	...
	if (rfx_pol->tunables->gaming_mode) {
		...
		if (rfx_pol->policy) {
			unsigned long cap = rfx_arch_cap(
...
static struct cpufreq_governor vorpal_gov = {
	.name   = "vorpal",
	.owner  = THIS_MODULE,
	/* 4.19's struct cpufreq_governor uses a plain bool, not the newer
	 * .flags bitmask (CPUFREQ_GOV_DYNAMIC_SWITCHING was introduced later
	 * when dynamic_switching was folded into a generic flags field). */
